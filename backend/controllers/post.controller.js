...
export const deletePost = async (req, res) => {
  try {
    ...
    if (img.error) {
      ...
      return res
        .status(500)
        .json({ message: "Cloudinary deletion error", success: false });
    }
...
export const unlikePost = async (req, res) => {
  try {
    ...
    await Post.findByIdAndUpdate(
     ...
     // Implement socket.io to send real-time unlike updates
    const user = await User.findById(userId).select("username profilePicture");
    const postOwnerId = post.author.toString();
    if (postOwnerId !== userId) {
      // Avoid sending unlike notification to yourself
      const notification = {
        type: "Unlike",
        userId,
        userDetailss:user,
...
export const getCommentsOfPost = async (req, res) => {
  try {
    ...
    if (comments.length === 0) {
      return res
        .status(404)
        .json({ message: "No comments on this post", success: false });
    }
    return res
      .status(200)
      .json({
